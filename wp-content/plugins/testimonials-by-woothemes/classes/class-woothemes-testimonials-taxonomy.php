...
class Woothemes_Testimonials_Taxonomy {
	...
	public function __construct ( $token = 'testimonial-category', $singular = '', $plural = '', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = __( 'Línea de Investigación', 'woothemes-testimonials' );
		if ( '' == $this->plural ) $this->plural = __( 'Líneas de Investigación', 'woothemes-testimonials' );
	...
	private function _get_default_labels () {
		return array(
			    ...
			    'all_items'           => sprintf( __( 'Todas %s', 'woothemes-testimonials' ), $this->plural ),
			    'parent_item'         => sprintf( __( 'Derivado de ( %s )', 'woothemes-testimonials' ), $this->singular ),
			    'parent_item_colon'   => sprintf( __( 'Derivado de( %s: )', 'woothemes-testimonials' ), $this->singular ),
			    'edit_item'           => sprintf( __( 'Editar %s', 'woothemes-testimonials' ), $this->singular ),
			    'update_item'         => sprintf( __( 'Actualizar %s', 'woothemes-testimonials' ), $this->singular ),
			    'add_new_item'        => sprintf( __( 'Añadir nueva %s', 'woothemes-testimonials' ), $this->singular ),
			    'new_item_name'       => sprintf( __( 'Nuevo %s Nombre', 'woothemes-testimonials' ), $this->singular ),
